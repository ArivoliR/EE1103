...
Description: Program uses Weighted Quick Union-Find with path compression.
...
int connect(int x, int y, DisjointSet* set) {
    ...
    if (rootX != rootY) {
        if (set->size[rootX] < set->size[rootY]) {
...
int main(int argc, char *argv[]) {
    ...
    FILE *output_file = fopen("ee23b008_quiz2_q3_output.txt", "w");
    if (output_file == NULL) {
